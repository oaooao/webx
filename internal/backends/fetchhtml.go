...
// FetchHTMLStd downloads a URL using Go's standard HTTP client (no uTLS).
// This client supports HTTP/2 natively and should be used for sites that
// don't need TLS fingerprint spoofing but may require h2 (e.g. YouTube).
func FetchHTMLStd(rawURL string) (string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), fetchHTMLTimeout)
	defer cancel()
